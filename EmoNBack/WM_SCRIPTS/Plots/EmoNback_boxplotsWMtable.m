...
%catch
    ...
    csvfile = dir(fullfile(file(3).folder,file(3).name,'EmoNback_WMBehaviorABCD*.csv'));
    ...
    %WM = importWMtable(fullfile(csvfileWM(3).folder,csvfileWM(3).name));
%end
close all

%saveDir = fullfile(pwd,'Plots/Boxplots',datestr(now,'yyyymmdd')); % Save data to designated directory
saveDir = fullfile(file(3).folder,file(3).name); % Save data to designated directory
...
plotLabels = {'0-back','2-back','Run 1','Run 2'};
%Overall = [Overall0back_ACC,Overall2back_ACC];
%plotLabels = {'0-back','2-back'};
...
bp1 = boxplot(Overall(:,1:2),'Labels',plotLabels(1:2),'Color',[0 0 0]); % Create boxplot
...
set(gca,'FontSize',32,'FontName','Avenir','FontWeight','b')
title(sprintf('Emotional Nback Working Memory: Overall Accuracy\nN = %1.0f',length(Overall)),'Fontsize',30)
...
saveas(fig(1),fullfile(saveDir,'Boxplots/WM',sprintf('EmoNbackWM_Overall_Accuracy_Block')),'jpeg')
hold off
%% WM 
fig(2) = figure(2); % Figure handle
set(fig(2),'Position',[1 100 1600 1600]); % Determine dimensions for figure

hold on
scatter(ones(size(Overall(:,3))),Overall(:,3),'o','MarkerEdgeColor',gray_matrix(gray_index,:),'LineWidth',1,'Jitter','on','JitterAmount',0.03)
scatter(ones(size(Overall(:,4)))*2,Overall(:,4),'o','MarkerEdgeColor',gray_matrix(gray_index,:),'LineWidth',1,'Jitter','on','JitterAmount',0.03)
bp2 = boxplot(Overall(:,3:4),'Labels',plotLabels(3:end),'Color',[0 0 0]); % Create boxplot
ylim([0 1.1]) % Set the y-axis limits [ymin ymax]
%annotation('textbox',[.15,.3,.3,.3],'String',sprintf('N = %1.0f',length(Overall)),'FitBoxToText','on');
...
%plot(3,Overall(:,3)','o','Color',gray_matrix(gray_index,:),'LineWidth',2)%[.01, .01, .01]*65,'LineWidth',2)
%plot(4,Overall(:,4)','o','Color',gray_matrix(gray_index,:),'LineWidth',2)%[.01, .01, .01]*65,'LineWidth',2)
saveas(fig(2),fullfile(saveDir,'Boxplots/WM',sprintf('EmoNbackWM_Overall_Accuracy_Runs')),'jpeg')
hold off
...
for i = 1:length(difSites)
    ...
    if length(ind)>3
        ...
        hold on
        bp2 = boxplot(Overall(ind,:),'Labels',plotLabels(1:end));
        ylim([0 1.1])
        set(bp2,'LineWidth',2)
        set(bp2(7,:),'Visible','off')
        set(gca,'FontSize',16)
        ...
        plot(1,Overall(ind,1)','o','Color',gray_matrix(gray_index,:),'LineWidth',2)%[.01, .01, .01],'LineWidth',2)
        plot(2,Overall(ind,2)','o','Color',gray_matrix(gray_index,:),'LineWidth',2)%[.01, .01, .01],'LineWidth',2)
